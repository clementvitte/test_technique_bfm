...
Le test n'est maheureusement pas terminé... Cependant toute la partie back fonctionne. 
...
Pour être plus précis, je pense que le blocage que j'ai est du aux varibles. Je stock mon nom dans une variable dans une fonction, sauf que je n'ai pas encore trouver la solution de comment utilisé une variable d'une fonction dans une autre fonction.
...
Pour lancer ce projet, il faut tout lancer le front et le back séparément. Tout d'abors, on doit lancer le server avec : 
...
CREATE TABLE name;
```
- Insertion des noms dans la base de données. Pour cette étape ce n'est pas une commande MySql mais une variable créer dans le fichier .js qui va ensuite envoyé toutes les données lors de la compilation.
...
Ensuite, le fichier index.js contient toutes les fonctions utile au fonctionnement du back du projet. 

Il contient l'API ainsi que la fonction "apiResponse(name)" permettant de faire les requête. Cette fonction retourne le resultat des requête dans le terminal.

La fonction "apiSql()" sert à récupérer un nom de la base de données et à l'initialisé comme paramètre à la fonction précédente.
...
J'ai actuellement codé ce test en 35H, et on peut remarqué que ce n'est pas suffisant pour le terminer...

L'objectif actuelle est de développer un peu le front, c'est à dire de réussir à afficher le nom sur la page HTML ansi que de créer le système de score. Le design de la page HTML n'est pas fait non plus, même si cela ne devrai pas prendre beacoup de temps mais j'ai préféré me concentré sur la partie back en premier. 
